...
function MobileCard({ row }) {
  ...
  // Each field gets its own alternating bg based on its own index
  let fieldIndex = 0;
  const Field = ({ label, children }) => {
    const bg = fieldIndex % 2 === 0 ? "bg-white" : "bg-[#F9FAFC]";
    fieldIndex++;
    return (
      <div className={`flex items-center justify-between py-3 px-4 border-b border-gray-100 ${bg}`}>
  ...
  return (
    <div className="rounded-2xl border border-border overflow-hidden mb-3">
      ...
      {/* Always-visible fields — each gets its own alternating color */}
      ...
      {/* Expandable fields — index continues from where we left off */}
      {expanded && (
        <>
