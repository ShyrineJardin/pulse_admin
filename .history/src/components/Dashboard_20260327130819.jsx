  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      <div className="bg-white rounded-2xl w-full max-w-md mx-4 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center gap-3 px-6 py-4 bg-[#F9FAFC] border-t border-gray-200">
          <button onClick={handleReset}
            className="flex-1 py-3 rounded-xl border border-primary text-primary text-sm font-medium bg-white hover:bg-green-50 transition-colors">
            Reset
          </button>
...
// ── Mobile Row Card ───────────────────────────────────────────
const ROW_BG = ["bg-white", "bg-[#F9FAFC]"];

function Field({ label, index, children }) {
  return (
    <div className={`flex items-center justify-between py-3 px-4 border-b border-gray-100 ${ROW_BG[index % 2]}`}>
    ...
    <div className="rounded-2xl border border-border overflow-hidden mb-3">
      <div className="flex items-center justify-between px-4 py-3 bg-white border-b border-gray-100">
        ...
        <div className="relative" ref={menuRef}>
          ...
          {menuOpen && (
            <div className="flex flex-col gap-2 absolute right-0 top-8 z-20 p-2 bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden w-44">
              <button className="px-3 py-1 text-sm border border-gray-200 rounded-md hover:bg-gray-100">Details</button>
      ...
      <Field label="Client Reference"    index={0}>{row.clientRef}</Field>
      <Field label="Instapay Reference"  index={1}>{row.instapayRef}</Field>
      <Field label="Provider Reference"  index={2}>{row.providerRef}</Field>
      <Field label="Merchant"            index={3}><span className="truncate max-w-[160px] block">{row.merchant}</span></Field>
      <Field label="Provider"            index={4}><span className="truncate max-w-[160px] block">{row.provider}</span></Field>
      <Field label="Type"                index={5}>
        <span className={`px-3 py-1 rounded-full text-xs font-medium ${TYPE_STYLES[row.type]}`}>{row.type}</span>
      </Field>
      <Field label="Status"              index={6}>
        <span className={`px-3 py-1 rounded-full text-xs font-medium ${STATUS_STYLES[row.status]}`}>{row.status}</span>
      </Field>
      <Field label="Amount"              index={7}>{row.amount}</Field>

      {expanded && (
        <>
          <Field label="Fees"         index={8}>{row.fees}</Field>
          <Field label="Error"        index={9}>{row.error}</Field>
          <Field label="Paid Date"    index={10}>{row.paidDate}</Field>
          <Field label="Created Date" index={11}>{row.createdDate}</Field>
        </>
      )}
      ...
      <button
        ...
        className="w-full py-3 text-sm font-medium text-gray-800 hover:text-dark-primary text-center border-t border-gray-200 bg-white hover:bg-gray-100 transition-colors"
      >
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="block md:hidden px-4 pt-4">
        {pageData.map((row, i) => <MobileCard key={i} row={row} />)}
        <Pagination />
      </div>
