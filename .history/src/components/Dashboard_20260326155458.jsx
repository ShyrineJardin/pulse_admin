...
function MobileCard({ row, i }) {
  ...
  // odd rows (i=0,2,4...) → white; even rows (i=1,3,5...) → #F9FAFC
  const isOdd = i % 2 === 0;
  ...
  // Each field row alternates: odd card → white header, #F9FAFC fields; even card → #F9FAFC header, white fields
  const headerBg = isOdd ? "bg-white" : "bg-[#F9FAFC]";
  const fieldBg  = isOdd ? "bg-[#F9FAFC]" : "bg-white";
  const footerBg = isOdd ? "bg-white" : "bg-[#F9FAFC]";

  const Field = ({ label, children }) => (
    <div className={`flex items-center justify-between py-3 px-4 border-b border-gray-100 ${fieldBg}`}>
  ...
  return (
    ...
    >
      {/* Card Header */}
      <div className={`flex items-center justify-between px-4 py-3 border-b border-gray-100 ${headerBg}`}>
        <div className="flex items-center gap-2">
        ...
        <div className="relative" ref={menuRef}>
          ...
          {menuOpen && (
            <div className="absolute right-0 top-8 z-20 bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden w-44">
              <button className="w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50">
                Details
              </button>
              <button className="w-full text-left px-4 py-2.5 text-sm text-white bg-primary hover:bg-dark-primary">
                Resend Webhook
              </button>
            </div>
          )}
      ...
      <Field label="Merchant">
        <span className="truncate max-w-[160px] block">{row.merchant}</span>
      </Field>
      <Field label="Provider">
        <span className="truncate max-w-[160px] block">{row.provider}</span>
      </Field>
      <Field label="Type">
      ...
      <button
        ...
        className={`w-full py-3 text-sm font-bold text-gray-800 text-center border-t border-gray-200 ${footerBg} hover:brightness-95 transition-all`}
      >
...
export default function Dashboard() {
  ...
  const Pagination = () => (
    <div className="flex items-center justify-center gap-2 mt-5 mb-5 flex-wrap">
      <button onClick={() => goPage(currentPage - 1)} disabled={currentPage === 1}
        className="flex items-center gap-1 px-3 py-2 text-xs md:text-sm border border-gray-200 rounded-lg text-gray-500 hover:bg-primary hover:text-btn-text disabled:opacity-40 disabled:cursor-not-allowed">
        ← Previous
      </button>
      {getPaginationPages().reduce((acc, page, idx, arr) => {
        if (idx > 0 && page - arr[idx - 1] > 1)
          acc.push(<span key={`e-${page}`} className="px-1 text-gray-400">...</span>);
        acc.push(
          <button key={page} onClick={() => goPage(page)}
            className={`w-9 h-9 text-sm rounded-lg border ${currentPage === page ? "bg-primary text-btn-text border-primary" : "border-gray-200 text-gray-700 hover:bg-gray-50"}`}>
            {page}
          </button>
        );
        return acc;
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* ── Header (unchanged) ── */}
      <div className="flex items-start md:items-center flex-col md:flex-row justify-between mb-6 bg-[#F9FAFC] w-full p-4 border-b border-gray-200">
