...
function MobileCard({ row, i }) {
  ...
  return (
    <div className={`rounded-2xl border-2 border-dashed border-blue-300 overflow-hidden mb-3`}>
      {/* Card Header */}
        ...
        <div className="relative" ref={menuRef}>
          ...
          {menuOpen && (
            <div className="absolute right-0 top-8 z-20 bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden w-44">
              <button className="w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50">Details</button>
              <button className="w-full text-left px-4 py-2.5 text-sm text-white bg-primary hover:bg-dark-primary">Resend Webhook</button>
            </div>
          )}
        </div>
      </div>
...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* ── Header ── */}
      <div className="bg-[#F9FAFC] border-b border-gray-200 px-4 py-3">
        {/* Top row: title + action buttons */}
        <div className="flex items-center justify-between gap-2 mb-3">
          <h1 className="text-base font-bold text-gray-900 leading-tight">PulseTech Performance Report</h1>
          <div className="flex items-center gap-2 flex-shrink-0">
            <button onClick={() => setShowFilter(true)}
              className="flex items-center gap-1.5 px-2.5 py-2 text-sm bg-white border border-gray-200 rounded-lg text-gray-600 hover:scale-105 transition-transform">
              <FaFilter className="text-gray-400 text-xs" />
            </button>
            <button className="flex items-center gap-1.5 px-3 py-2 text-sm bg-gray-900 text-primary rounded-lg hover:bg-gray-800 whitespace-nowrap">
        ...
        <div className="relative w-full">
          ...
          <input type="text" value={search} onChange={handleSearch}
            ...
            className="w-full pl-8 pr-4 py-2 text-sm border border-gray-200 rounded-lg outline-none focus:border-gray-400 bg-white" />
        </div>
      </div>
